...
class UserController
{
  ...
  public function findAll(ServerRequestInterface $request): ResponseInterface
  {
    ...
    // On passe '/users' comme page courante pour activer le bon menu
    return new Response(
      200,
      ['Content-Type' => 'text/html'],
      View::baseTemplate($title, $html, '/users')
    );
  }
  ...
  public function displayAddForm(ServerRequestInterface $request): ResponseInterface
  {
    ...
    // On passe '/users' car cette page fait partie de l'administration des utilisateurs
    return new Response(
      200,
      ['Content-Type' => 'text/html'],
      View::baseTemplate('Ajout d\'un utilisateur', '<h1>Ajout d\'un utilisateur</h1>' . $html, '/users')
    );
  }
  ...
  public function add(ServerRequestInterface $request): ResponseInterface
  {
    ...
    // Utilisation d'une page de confirmation avec redirection automatique
    $html = View::baseTemplate(
      ...
      '<div class="alert alert-success" role="alert">
        ...
        <hr>
        <p class="mb-0">Redirection automatique dans 2 secondes...</p>
      </div>
      <script>
        setTimeout(function() {
  ...
  public function delete(ServerRequestInterface $request, array $routeParams = []): ResponseInterface
  {
    ...
    if (isset($id)) {
      ...
      if (!$delete) {
        return new Response(
          418,
          ['Content-Type' => 'text/html'],
          View::baseTemplate('Erreur', '<h1>Problème dans la suppression de l\'utilisateur</h1>', '/users')
        );
      }

      // Redirection vers /users après suppression
      return new Response(
        302,
        ['Location' => '/users'],
        null
    ...
    return new Response(
      400,
      ['Content-Type' => 'text/html'],
      View::baseTemplate('Erreur', '<h1>La requête HTTP a été mal formulée</h1>', '/users')
    );
  }
    ...
    if (isset($id)) {
      ...
      if (!$this->userRepository->save($user)) {
        return new Response(
          418,
          ['Content-Type' => 'text/html'],
          View::baseTemplate('Erreur', '<h1>Problème dans la mise à jour</h1>', '/users')
        );
      }
    ...
    if (isset($id)) {
      ...
      if (!$user) {
        return new Response(
          404,
          ['Content-Type' => 'text/html'],
          View::baseTemplate('Erreur', '<h1>Aucun utilisateur ne correspond !</h1>', '/users')
        );
      }
